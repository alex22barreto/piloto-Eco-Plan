...
describe('Estado e2e test', () => {
  ...
  it('should create and save Estados', async () => {
    ...
    await promise.all([estadoUpdatePage.setEstadoNombreInput('estadoNombre')]);

    expect(await estadoUpdatePage.getEstadoNombreInput()).to.eq('estadoNombre', 'Expected EstadoNombre value to be equals to estadoNombre');
