...
export class TipoActividadUpdatePage {
  ...
  idTipoActividadInput = element(by.id('field_idTipoActividad'));
  ...
  async setIdTipoActividadInput(idTipoActividad: string): Promise<void> {
    await this.idTipoActividadInput.sendKeys(idTipoActividad);
  }

  async getIdTipoActividadInput(): Promise<string> {
    return await this.idTipoActividadInput.getAttribute('value');
  }
