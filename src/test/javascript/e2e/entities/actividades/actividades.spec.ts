...
describe('Actividades e2e test', () => {
  ...
  it('should create and save Actividades', async () => {
    ...
    await promise.all([
      ...
      actividadesUpdatePage.setEstadoInput('estado'),
      actividadesUpdatePage.setTipoInput('tipo')
    ]);
    ...
    expect(await actividadesUpdatePage.getEstadoInput()).to.eq('estado', 'Expected Estado value to be equals to estado');
    expect(await actividadesUpdatePage.getTipoInput()).to.eq('tipo', 'Expected Tipo value to be equals to tipo');
